...
vector<Point> getHull(vector<Point> &points) {
    swap(points[getPivotIndex(points)], points.front());
    size_t size = points.size();
    for (size_t i = 1; i < size; ++i) {
        points[i].r = Vector{points[i].x - points.front().x,
                             points[i].y - points.front().y};
    }
    ...
    for (size_t i = 2; i < size; ++i) {
        ...
        while (hull.size() >= 2 && vectorProduct(
                Vector{hull[hull.size() - 2].x - hull.back().x,
                       hull[hull.size() - 2].y - hull.back().y},
...
long double getSquare(const vector<Point> &polygon) {
    ...
    size_t j = 0;
    for (auto &v : vectors) {
        v = Vector{polygon[j].x, polygon[j].y};
        ++j;
    }
    vectors.push_back(vectors.front());
    long double result = 0.0;
    for (size_t i = 1; i < vectors.size(); ++i) {
        result += getTriangleSquare(vectors[i - 1], vectors[i]);
    }
    return abs(result);
}