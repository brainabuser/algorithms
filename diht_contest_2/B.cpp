...
int extractByIndex(int *heap, int &heap_size, int index);

enum RequestType {
    DELETE = 1, INSERT, DELETE_BY_INDEX
};
    ...
    for (int i = 0; i < request_number; ++i) {
        ...
        switch (request) {
            ...
            case DELETE_BY_INDEX:
                cin >> temp;
                cout << extractByIndex(heap, heap_size, temp) << endl;
                break;
        }
    }
...
int insert(int *heap, int &heap_size, int capacity, int value) {
    if (heap_size >= capacity) {
        ...
        return siftUp(heap, heap_size - 1);
    }
}
...
int extractByIndex(int *heap, int &heap_size, int index) {
    ...
    int new_index = siftUp(heap, index);
    --new_index;
    siftDown(heap, new_index, heap_size);
    return elem;
}